...
class GameEngine:
    def __init__(self, walls):
        ...
        self.state_update_min_period = 0.1  # 10 times per sec
    ...
    def update_state_on_server(self):
        ...
        # Add debugging
        print(f"Sending state with {len(self.coins)} coins")
        
        # Send in background thread to avoid blocking game loop
        def send_state():
            try:
                requests.post(SERVER_URL + "/update_state", json=state, timeout=0.5)
            except Exception as e:
    ...
    def get_agent_actions(self, game_state, walls):
        """
        Gets agent actions using only sensor data through the server
        """
        try:
...
class Coin:
    def __init__(self, x, y):
        ...
        self.rect = pygame.Rect(self.x - 8, self.y - 10, 16, 20)  # Larger coin size
...
def main():
    ...
    while running:
        ...
        # Check if the player collects a coin - use slightly more generous collision detection
        player_rect = pygame.Rect(player.x - 20, player.y - 20, 40, 40)  # Even larger for easier collection
