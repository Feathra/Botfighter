...
MINIMAP_WIDTH = SCREEN_WIDTH/4  # Adjust as needed
...
def draw_minimap(screen, engine, walls):
  # Create a separate surface for the minimap
    minimap_surface = pygame.Surface((MINIMAP_WIDTH, MINIMAP_HEIGHT))
    minimap_surface.fill(GRAY)  # Black background for minimap
    ...
    for wall in walls:
        ...
        minimap_surface,
        (80, 80, 80),
        (wall.x * scale_x, wall.y * scale_y, wall.width * scale_x, wall.height * scale_y),
        )
  ...
  # Draw ships on the minimap
    for i, ship in enumerate(engine.ships):
        ...
        minimap_surface,
        color,
        (int(ship.x * scale_x), int(ship.y * scale_y)),
        3,
        )  # Smaller circles for ships
 

  # Draw player's view area on the minimap
    player = engine.ships[0] if engine.ships else None
    if player:
        pygame.draw.circle(
        minimap_surface,
        BLUE,
        (int(player.x * scale_x), int(player.y * scale_y)),
        8,
        1  # 1 pixel border
        )
...
def main():
    ...
    while True:
        ...
        while running:
            ...
            # Draw the minimap
            draw_minimap(screen, engine, walls)
