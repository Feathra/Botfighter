...
class SpaceObject:
    ...
    def __init__(self, x, y, angle=0, velocity_x=0, velocity_y=0, hp=100, concurrents=None):
...
def fetch_walls():
    try:
        ...
        print(f"Error fetching walls from server: {e}")
        return None
...
MINIMAP_WIDTH = SCREEN_WIDTH/3.5  # Adjust as needed
MINIMAP_HEIGHT = SCREEN_HEIGHT/2.5
MINIMAP_X = 0
MINIMAP_Y = SCREEN_HEIGHT - MINIMAP_HEIGHT




def draw_minimap(screen, game_state, walls):
    # Create a separate surface for the minimap
    minimap_surface = pygame.Surface((MINIMAP_WIDTH, MINIMAP_HEIGHT))
    minimap_surface.fill(BLACK)  # Use BLACK as background for minimap
    ...
    def scale_point_minimap(x, y):
        return x * minimap_scale, y * minimap_scale

    def scale_rect_minimap(rect):
        x, y = scale_point_minimap(rect.x, rect.y)
        width = int(rect.width * minimap_scale)
        height = int(rect.height * minimap_scale)
        return pygame.Rect(x, y, width, height)

    # Draw walls onto the minimap_surface
    for wall in walls:
        rect = scale_rect_minimap(wall)
        pygame.draw.rect(minimap_surface, GRAY, rect)

    # Draw ships onto the minimap_surface
    for ship in game_state.get("ships", []):
        pos = pygame.math.Vector2(*scale_point_minimap(ship["x"], ship["y"]))
        angle = ship["angle"]
        points = [
            pos + pygame.math.Vector2(40 * minimap_scale, 0).rotate_rad(angle),
            pos + pygame.math.Vector2(-20 * minimap_scale, 20 * minimap_scale).rotate_rad(angle),
            pos + pygame.math.Vector2(-20 * minimap_scale, -20 * minimap_scale).rotate_rad(angle),
        ]
        pygame.draw.polygon(minimap_surface, BLUE, points)

    # Draw bullets onto the minimap_surface
    for bullet in game_state.get("bullets", []):
        x, y = scale_point_minimap(bullet["x"], bullet["y"])
        pygame.draw.circle(minimap_surface, RED, (int(x), int(y)), max(1, int(2 * minimap_scale)))

    # Draw coins onto the minimap_surface
    for coin in game_state.get("coins", []):
        x, y = scale_point_minimap(coin["x"], coin["y"])
        pygame.draw.circle(minimap_surface, YELLOW, (int(x), int(y)), max(1, int(10 * minimap_scale)))

    # Blit the entire minimap_surface onto the main screen
    ...
    while True:
        ...
        # Fetch game data from the server
            ...
            if now - last_update_time > update_interval:
                try:
                    if ACCESS_SERVER_FLAG:
                        fetched_walls = fetch_walls()
                        if fetched_walls:
                            current_walls = fetched_walls

                        fetched_game_state = fetch_game_state()
                        if fetched_game_state:
                            game_state = fetched_game_state
                    else:
                        fetched_game_state = fetch_game_state()
                        if fetched_game_state:
                            game_state = fetched_game_state
            ...
            for wall in walls:
                ...
                pygame.draw.rect(screen, GRAY, wall_screen)
            # print(f"Wall world coords: ({wall.x}, {wall.y}), screen coords: ({wall_screen.x}, {wall_screen.y})")
            ...
            # Draw the minimap
            draw_minimap(screen, game_state, walls)
